      ...
      <div className="grid md:grid-cols-2 gap-8">
        ...
        <div>
          ...
          <div className="flex items-center gap-2 mb-4">
            ...
            <span className="font-mono text-gray-600" data-testid="product-part-number">{part.part_number}</span>
          </div>
            ...
            <div className="border border-gray-200 rounded-xl p-4 mb-6">
              <div className="flex items-start gap-4">
                <div className="w-12 h-12 bg-[#1a5c38]/10 rounded-full flex items-center justify-center flex-shrink-0">
                  <span className="text-[#1a5c38] font-bold text-lg">{part.seller.name?.charAt(0)}</span>
                </div>
                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-2">
                    <Link to={`/sellers/${part.seller.id}`}
                      className="font-semibold text-gray-900 hover:text-[#1a5c38]" data-testid="seller-link">
                      {part.seller.name}
                    </Link>
                    {part.seller.verified && <CheckCircle size={16} className="text-[#1a5c38]" />}
                  </div>
          ...
          {isAuthenticated && (
            <button
              onClick={() => toggleFavorite(part.id)}
              className={`mt-3 w-full flex items-center justify-center gap-2 py-2 rounded-lg border transition-colors text-sm font-medium ${
                isFavorite
                ...
                {part.price_comparison.map((comp) => {
                  ...
                  return (
                    <TableRow key={comp.id}>
                      <TableCell>
                        <div className="flex items-center gap-2">
                          <Link to={`/sellers/${comp.seller?.id}`} className="font-medium hover:text-[#1a5c38]">
                            {comp.seller?.name}
                          </Link>
                          {comp.seller?.rating && (
