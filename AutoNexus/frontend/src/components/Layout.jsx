...
const Layout = () => {
  ...
  // `testId` stays a stable English slug (independent of the active
  // language) so data-testid attributes below don't change when the user
  // switches languages -- only `label` is translated.
  const navItems = [
    { path: "/", icon: Home, testId: "home", label: t("nav.home") },
    { path: "/search", icon: Search, testId: "search", label: t("nav.search") },
    { path: "/sellers", icon: Store, testId: "sellers", label: t("nav.sellers") },
                  ...
                  <DropdownMenuContent align="end" className="w-48">
                    ...
                    {isAuthenticated && (
                      <DropdownMenuItem onClick={() => navigate("/dashboard")} data-testid="dashboard-menu-item">
                        ...
                        {isSeller ? t("nav.dashboard") : t("nav.become_a_seller")}
                      </DropdownMenuItem>
                    )}
                    {isAdmin && (
            ...
            <nav className="px-4 py-2 space-y-1">
              ...
              {isAuthenticated && (
                ...
                >
                  ...
                  <span className="font-medium">{isSeller ? t("nav.dashboard") : t("nav.become_a_seller")}</span>
                </Link>
              )}
              {isAdmin && (
                <Link
                ...
                >
                  <Shield size={20} />
                  <span className="font-medium">{t("nav.admin_panel")}</span>
                </Link>
              )}
      ...
      <footer className="hidden md:block bg-[#0f2e1c] text-white mt-auto">
        <div className="max-w-7xl mx-auto px-4 py-12">
          <div className="grid grid-cols-4 gap-8">
            ...
            {/* Quick Links */}
            <div>
              <h4 className="font-semibold mb-4 text-lg">{t("footer.quick_links_title")}</h4>
              <ul className="space-y-2 text-gray-300">
                <li><Link to="/search" className="hover:text-white transition-colors">{t("footer.search_parts_link")}</Link></li>
                <li><Link to="/sellers" className="hover:text-white transition-colors">{t("footer.find_sellers_link")}</Link></li>
                <li><Link to="/requests" className="hover:text-white transition-colors">{t("footer.part_requests_link")}</Link></li>
                <li><Link to="/dashboard" className="hover:text-white transition-colors">{t("nav.become_a_seller")}</Link></li>
              </ul>
            </div>

            {/* Contact */}
            <div>
              <h4 className="font-semibold mb-4 text-lg">{t("footer.contact_title")}</h4>
